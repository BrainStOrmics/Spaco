import os
from .ios import scms, scmm, assign_color

def scm(data, color = None, atype = None,  outdir = "scm_output",
        radius = 90, knn = 8, ncell = 3, method = 'manhattan', cycle = 10000, size = 40):
  ...
  if (len(data.split(',')) == 1):
    scms(data, color, dtype, dcolor, outdir, radius, knn, ncell, method, cycle, size)
  else:
    scmm(data, color, dtype, dcolor, outdir, radius, knn, ncell, method, cycle, size)
