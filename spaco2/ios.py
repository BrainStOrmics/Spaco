...
def figs(adata, palet, prefix, size):
    ...
    """
    sc.pl.spatial(
        adata, color="annotation", palette=palet, spot_size=1, size=size, save=prefix
    )
...
def rdat(data, type):
    ...
    if not "spatial" in adata.obsm or not "annotation" in adata.obs:
        print(
            data
            + "without adata.obsm['spatial'] or adata.obs['annotation'], please supplement them first"
        )
        sys.exit(-1)
...
def scms(
    ...
    sc.pl.spatial(
        adata, color="annotation", spot_size=1, size=size, save=f"_{sample}_raw.pdf"
    )
    if color == None:
        color = adata.uns["annotation_colors"]
    tsp_score, df_color = spatial_color(
        color,
    ...
    figs(
        ...
        f"_{sample}_r{radius}k{knn}c{cycle}.pdf",
        size,
    )
...
def scmm(
    ...
    for i in tmp:
        ...
        key, val = find_neighbors(
            np.array(adata.obsm["spatial"]), adata.obs["annotation"], radius, knn, ncell
        )
    ...
    if len(color) < len(mg_key):
        ...
        sys.exit(-1)
    tsp_score, df_color = spatial_color_cal(
        mg_key, np.array(mg_mat), color[0 : len(mg_key)], method, cycle
    )
    tsp_fig(tsp_score, f"{outdir}/tsp_r{radius}k{knn}c{cycle}.png")
    df_color = pd.concat([df_color, dcolor])
    ...
    for i in tmp:
        ...
        ss_df_color.to_csv(
            f"{outdir}/{sample}_r{radius}k{knn}c{cycle}.csv", index=False
        )
        figs(
            adata,
            ss_df_color["color"].tolist(),
            f"_{sample}_r{radius}k{knn}c{cycle}.pdf",
            size,
        )
